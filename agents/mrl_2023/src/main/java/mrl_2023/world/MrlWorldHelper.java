...
import mrl_2023.SampleModuleKey;
import mrl_2023.complex.firebrigade.BorderEntities;
import mrl_2023.complex.firebrigade.BuildingProperty;
import mrl_2023.complex.firebrigade.MrlFireBrigadeWorld;
import mrl_2023.util.Util;
import mrl_2023.world.entity.*;
import mrl_2023.world.helper.*;
import mrl_2023.world.routing.graph.GraphModule;
...
//            System.err.println(" getting MrlWorldHelper .......");
            if (worldInfo.getEntity(agentInfo.getID()) instanceof FireBrigade) {
                worldHelper = moduleManager.getModule(SampleModuleKey.AGENT_WORLD, "mrl_2023.complex.firebrigade.MrlFireBrigadeWorld");
            } else {
                worldHelper = moduleManager.getModule(SampleModuleKey.AGENT_WORLD, "mrl_2023.world.MrlWorldHelper");
            }
        } catch (Exception ex) {
            ex.printStackTrace();
            worldHelper = null;
...
//                Property brokennessProperty = building.getProperty(Integer.parseInt(StandardPropertyURN.BROKENNESS.toString()));

                Property brokennessProperty = building.getProperty(StandardPropertyURN.BROKENNESS.getURNId());
                //Change to Get URN ID -Plese Change this to other class not Use Parse
...
//                        newBrokennessValue = (Integer) p.getValue();
                        ...
                        //Man Dar Avordi
                        if (p.getURN()==(brokennessProperty.getURN())) {
                            if (p.getURN() == (brokennessProperty.getURN())) {
