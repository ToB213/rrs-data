...
public class CSUATHumanSystem extends AbstractModule {
    ...
    public CSUATHumanSystem(CSUWorldHelper csuWorldHelper,AgentInfo ai, WorldInfo wi, ScenarioInfo si, ModuleManager moduleManager, DevelopData developData, Clustering clustering, PathPlanning pathPlanning) {
        ...
        this.maxDistance =(int)(Math.sqrt(csuWorldHelper.getMapHeight()*csuWorldHelper.getMapWidth()/si.getScenarioAgentsAt())*1.5);
    }
        ...
        for (EntityID entityID : changed){
            ...
            if (entity instanceof FireBrigade){
                ...
                for (EntityID entityID1 : this.sendCommandPolice){
                    messageManager.addMessage( new CommandPolice( false, StandardMessagePriority.HIGH,null,
        ...
        for (Civilian civilian : this.allCivilian.keySet()){
            if (civilian.isHPDefined() && civilian.getHP()>0 ){
                ...
                if (civilianPosition instanceof Building){
                    ...
                    if (  (civilian.isDamageDefined()&&civilian.getDamage()>0||building.isBrokennessDefined()&&building.getBrokenness()>0)){
