  ...
  public static int[] execute(int[][] mat) {
    ...
    for (int r = 0; r < n; ++r) {
      ...
      for (; ; ) {
        ...
        for (int j = 0; j < m; ++j) {
          if (!right[j]) {
            d = Math.min(d, residue(mat, ofsLeft, ofsRight, ptr[j], j));
          }
        }
        for (int i = 0; i < n; ++i) {
          if (left[i]) {
        ...
        int b = -1;
        for (int j = 0; j < m; ++j) {
          if (!right[j] && residue(mat, ofsLeft, ofsRight, ptr[j], j) == 0) {
            b = j;
          }
        }
        trace[b] = ptr[b];
        ...
        right[b] = left[c] = true;
        for (int j = 0; j < m; ++j) {
          if (residue(mat, ofsLeft, ofsRight, c, j) < residue(mat, ofsLeft, ofsRight, ptr[j],
              j)) {
            ptr[j] = c;
          }
        }
