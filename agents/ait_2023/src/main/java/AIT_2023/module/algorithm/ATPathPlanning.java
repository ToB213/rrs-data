...
public class ATPathPlanning extends PathPlanning {
  ...
  public ATPathPlanning(AgentInfo ai, WorldInfo wi, ScenarioInfo si, ModuleManager mm,
      DevelopData dd) {
    super(ai, wi, si, mm, dd);

    this.highways = mm.getModule("AIT.Algorithm.ATPathPlanning.Highways");
    this.registerModule(this.highways);

    this.failedMove = mm.getModule("AIT.Algorithm.ATPathPlanning.FailedMove");
    this.registerModule(this.failedMove);

    this.stuckedHumans = mm.getModule("AIT.Algorithm.ATPathPlanning.StuckHumans");
  ...
  public PathPlanning setDestination(Collection<EntityID> destinations) {
    ...
    EntityID targetRefuge = null;
    int distance = Integer.MAX_VALUE;

    boolean type = true;
    for (EntityID e : destinations) {
      if (!(this.worldInfo.getEntity(e) instanceof Refuge)) {
        type = false;
        break;
      }
    }
    ...
    if (type) {
      for (EntityID e : destinations) {
        int capacity = ((Refuge) this.worldInfo.getEntity(e)).getBedCapacity();
        int occupied = ((Refuge) this.worldInfo.getEntity(e)).getOccupiedBeds();
        if (capacity - occupied > 0) {
          int d = this.worldInfo.getDistance(me, e);
          if (d < distance) {
            targetRefuge = e;
            distance = d;
          }
...
//    }
  ...
  private Collection<Area> gatherRoads(Collection<EntityID> ids) {
    final Stream<Area> ret = ids.stream().map(this.worldInfo::getEntity)
  ...
  private void reflectEdgeStates(Area area) {
    ...
    final Geometry passable = GraphEdgeValidator.computePassable(area, blockades);

    final Stream<Couple<EntityID>> invalids = this.gatherEdges(area).stream()
  ...
  private boolean validateEdge(Area area, Geometry passable, Couple<EntityID> edge) {
    final Line2D location = new Line2D(this.getPoint(), this.getPoint());

    final Line2D l1 = edge.elem1 == null ? location : area.getEdgeTo(edge.elem1).getLine();
    final Line2D l2 = edge.elem2 == null ? location : area.getEdgeTo(edge.elem2).getLine();
